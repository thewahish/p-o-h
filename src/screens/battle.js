...
  const getResourcePercent = (p) => {
  ...
  // --- NEW: XP Bar Calculation ---
  const getXpPercent = (p) => {
    if (!p || p.level >= GameConfig.XP_CURVE.maxLevel) return 100;
    ...
    if (totalXpForLevel === 0) return 0; // Avoid division by zero
    return Math.max(0, Math.min(100, (xpIntoLevel / totalXpForLevel) * 100));
  };
...
        </div>
        ...
        {/* --- MODIFIED: Player Box with Portrait, Stats, and XP Bar --- */}
        <div className="bg-gray-800 p-4 rounded-lg border border-amber-600 mb-4">
          ...
          <div className="mt-3">
            <div className="h-2 bg-gray-700 rounded overflow-hidden mb-1">
                <div className="h-2 bg-cyan-400 transition-all duration-300" style={{ width: `${getXpPercent(player)}%` }}></div>
            </div>
            <div className="text-xs text-center text-cyan-200">XP</div>
          </div>
        </div>

        {/* --- MODIFIED: Enemy Boxes with Portraits and Stats --- */}
