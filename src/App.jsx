...
const ENEMIES = {
  GOBLIN: {
    name: "Goblin",
    icon: "👺",
    stats: { hp: 60, atk: 18, def: 6, spd: 8, pow: 5 },
  },
  SKELETON: {
    name: "Skeleton",
    icon: "💀",
    stats: { hp: 50, atk: 15, def: 8, spd: 6, pow: 8 },
  },
  ORC: {
    name: "Orc",
    icon: "👹",
    stats: { hp: 90, atk: 22, def: 10, spd: 4, pow: 6 },
  },
  WRAITH: {
    name: "Wraith",
    icon: "👻",
    stats: { hp: 70, atk: 12, def: 4, spd: 9, pow: 15 },
  },
};
...
const BOSSES = {
  ORC_WARLORD: {
    name: "Orc Warlord",
    icon: "👹",
    stats: { hp: 250, atk: 30, def: 15, spd: 6, pow: 10 },
  },
};
...
export default function PathOfHeroes() {
  ...
  const generateDungeon = () => {
    const newDungeon = Array(GRID_SIZE)
      .fill(null)
      .map(() =>
        Array(GRID_SIZE)
          .fill(null)
    ...
    path.forEach((pos) => {
      ...
      adjacents.forEach((adj) => {
        if (
          ...
          adj.y < GRID_SIZE &&
          Math.random() < 0.3 &&
          newDungeon[adj.y][adj.x].type === ROOM_TYPES.EMPTY
        ) {
          newDungeon[adj.y][adj.x].type =
            Math.random() < 0.7
              ? ROOM_TYPES.BATTLE
              : getRandomSpecialRoom();
        }
  ...
  const startBattle = (isElite) => {
    ...
    setBattleState({
      enemy,
      turn: "player",
      log: [`A${isElite ? "n elite" : ""} ${enemy.name} appears!`],
    });
    setGameState("battle");
  };
  ...
  const startBossBattle = () => {
    ...
    setBattleState({
      enemy: boss,
      turn: "player",
      log: [`The ${boss.name} blocks your path!`],
    });
    setGameState("battle");
  };
    ...
    return (
      <CampfireScreen
        ...
        onHeal={() =>
          setPlayer((prev) => ({
            ...prev,
            currentHp: Math.min(prev.baseStats.hp, prev.currentHp + 50),
          }))
        }
        onUpgrade={() =>
          setPlayer((prev) => ({
            ...prev,
            baseStats: { ...prev.baseStats, atk: prev.baseStats.atk + 2 },
          }))
        }
      />
    );
  ...
  if (gameState === "shop") {
    return (
      <ShopScreen
        gold={gold}
        onPurchase={(cost) => setGold((prev) => prev - cost)}
        onContinue={() => setGameState("exploration")}
      />
    );
  }
        ...
        {Object.values(CHARACTERS).map((char) => (
          ...
          >
            ...
            <div className="text-sm text-gray-400 mt-1">
              {char.resource}:{" "}
              <span style={{ color: char.resourceColor }}>
                {char.baseStats.resource}
              </span>
            </div>
          </button>
        ))}
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-4 flex flex-col items-center">
      ...
      <div className="flex space-x-8 mb-4">
        <div className="text-center">
          ...
          <div>{player.name}</div>
          <div>
            HP: {player.currentHp}/{player.baseStats.hp}
          </div>
        </div>
        <div className="text-center">
...
function CampfireScreen({ onContinue, onHeal, onUpgrade }) {
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-4 flex flex-col items-center">
      <h2 className="text-xl mb-4">Campfire</h2>
      <p className="mb-4">Rest or upgrade your abilities</p>
      <div className="space-x-4 mb-4">
        <button onClick={onHeal} className="bg-green-600 px-4 py-2 rounded">
          Heal
        </button>
        <button onClick={onUpgrade} className="bg-blue-600 px-4 py-2 rounded">
          Upgrade Attack
        </button>
      </div>
      <button onClick={onContinue} className="bg-gray-700 px-4 py-2 rounded">
        Continue
      </button>
    </div>
  );
...
function ShrineScreen({ onBlessingChosen }) {
  const blessings = ["Strength +5", "Speed +2", "Power +3"];
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-4 flex flex-col items-center">
