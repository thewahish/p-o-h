#!/usr/bin/env node
/**
 * FULL GAME COMPLETION SIMULATOR - Option 3 Balance
 *
 * Simulates a real player attempting to beat all 50 floors
 * Tracks every attempt, meta-progression, and time investment
 *
 * NEW BALANCE (Option 3 - Hybrid):
 * - 1 starting potion (not 2)
 * - Slower XP curve (80 + level*60, not 60 + level*40)
 * - Harder enemies (+40% HP, +40% ATK)
 * - Better Hero Souls (5 + floor/2, not 2)
 * - Item drops: 20% chance (reduced from 30%)
 * - Weaker items: +1-2 ATK, +0-1 DEF
 * - 50 total floors (expanded from 30)
 */

console.log('🎮 FULL GAME COMPLETION SIMULATOR');
console.log('Playing Path of Heroes from Run 1 to Final Boss Victory\n');
...
const BASE_ENEMIES = {
    goblin: { hp: 55, atk: 12, def: 3, spd: 6, crit: 5, xp: 20, gold: [2, 5] },
    slime: { hp: 65, atk: 11, def: 8, spd: 3, crit: 3, xp: 25, gold: [3, 6] },
    orcBrute: { hp: 85, atk: 16, def: 5, spd: 5, crit: 8, xp: 35, gold: [5, 10] }
};
...
function xpForLevel(level) {
    return 80 + (level - 1) * 60; // Slower leveling (Option 3)
}
...
function scaleEnemy(baseEnemy, floor) {
    const scaleFactor = 1 + (floor - 1) * 0.08; // +8% stats per floor
    return {
...
function simulateFloor(character, floor, metaSouls, unlockedUpgrades) {
    ...
    const player = {
        ...
        crit: base.crit,
        gold: 100,
        potions: 1, // Only 1 potion now (Option 3)
    ...
    for (let i = 0; i < encounterCount; i++) {
        ...
        // Shrine (10% chance, reduced from 20%)
        if (Math.random() < 0.1) {
            const boost = ['hp', 'atk', 'def'][rng(0, 2)];
            if (boost === 'hp') {
                player.maxHp += rng(3, 6);
                player.hp = Math.min(player.hp + rng(3, 6), player.maxHp);
            } else if (boost === 'atk') player.atk += rng(1, 3);
            else if (boost === 'def') player.def += rng(1, 2);
            totalTime += 5;
        }

        // Item drop (20% chance, reduced from 30%)
        if (Math.random() < 0.2) {
            player.atk += rng(1, 2); // Weaker items (was 1-4)
            player.def += rng(0, 1); // Weaker items (was 0-3)
            totalTime += 3;
        }
...
console.log('\n🎮 PLAYER JOURNEY: Attempting to beat all 50 floors...\n');
...
// ===== FINAL ANALYSIS =====
console.log('\n' + '='.repeat(70));
console.log('📊 FINAL ANALYSIS - PLAYER EXPERIENCE\n');
...
[1, 5, 10, 20, 30, 40, 50].forEach(floor => {
    ...
    if (victory) {
        console.log(`  Floor ${String(floor).padStart(2)}: Run #${victory.run} (${victory.time} minutes)`);
    }
});
...
if (firstClear) {
    ...
    if (firstClear.run <= 5) {
        console.log(`❌ Floor 1 too easy (cleared in ${firstClear.run} runs, ${timeMin} min)`);
    } else if (firstClear.run <= 20) {
        console.log(`✅ Floor 1 perfect (cleared in ${firstClear.run} runs, ${timeMin} min)`);
    } else {
        console.log(`⚠️  Floor 1 too hard (cleared in ${firstClear.run} runs, ${timeMin} min)`);
    }
}
...
if (fullComplete) {
    ...
    if (hours < 3) {
        console.log(`⚠️  Game too short (${hours}h to completion)`);
    } else if (hours <= 8) {
        console.log(`✅ Perfect length (${hours}h to completion - Target: 4-8h)`);
    } else {
        console.log(`⚠️  Game too long (${hours}h to completion)`);
    }
} else {
    console.log(`❌ Game incomplete after ${runNumber} runs (${Math.floor(totalPlayTime / 3600)}h)`);
}
