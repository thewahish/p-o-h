#!/usr/bin/env node
/**
 * Game Balance Simulator V4 - Option C with 1 HP Potion
 * Tests V.39.2 balance changes - Final roguelike difficulty with emergency potion
 */

console.log('🎮 Balance Simulator V4 - Testing V.39.2 (Option C + 1 Potion)\n');
console.log('='.repeat(70));

// Option C Character stats (V.39.2 - Custom balance)
// - Player HP reduced 20% (80/65/72)
// - Player ATK kept original (12/14/15)
// - 1 HP Potion (50 HP) as emergency healing
const characters = {
    warrior: { hp: 80, atk: 12, def: 10, spd: 6, crit: 10, resource: 50, potions: 1 },
    sorceress: { hp: 65, atk: 14, def: 6, spd: 7, crit: 12, resource: 70, potions: 1 },
    rogue: { hp: 72, atk: 15, def: 7, spd: 10, crit: 18, resource: 60, potions: 1 }
};
...
function runFullSimulation(characterType, simNumber) {
    ...
    for (let i = 0; i < floor1Combats; i++) {
        ...
        if (result.won) {
            ...
            // Use potion if HP is critically low (< 30%) and we have one
            const hpPercent = player.hp / characters[characterType].hp;
            if (hpPercent < 0.3 && player.potions > 0) {
...
console.log('\n📊 Running 25 Simulations (V.39.2 - Option C + 1 Potion)...\n');
...
for (let i = 1; i <= 25; i++) {
    ...
    const potionInfo = result.potionsUsed > 0 ? `Potion used` : '';
...
const totalPotionsUsed = simulations.reduce((sum, s) => s.potionsUsed, 0);
...
console.log(`Potions Used:       ${totalPotionsUsed}/25`);
...
if (actualSurvival >= 55 && actualSurvival <= 75) {
    ...
    console.log('   Emergency potion provides strategic depth.');
} else if (actualSurvival > 75) {
    console.log('\n⚠️  WARNING: Still slightly too easy');
    console.log('   Consider removing the starting potion.');
} else if (actualSurvival < 45) {
    console.log('\n⚠️  WARNING: Too difficult');
    console.log('   Consider adding 1 more potion or 5-10% HP buff).');
} else {
...
console.log('Sample Matchups (V.39.2 + 1 Potion):\n');
...
console.log('='.repeat(70));
console.log('\n✨ V.39.2 (Option C + 1 Potion) FINAL Balance Test Complete!\n');
