#!/usr/bin/env node
/**
 * ULTIMATE BALANCED SIMULATOR - Final Solution
 *
 * Two-part balance fix:
 * 1. Harder Floor 1: Enemies +10% HP/ATK from Option 4 (target: 10-20 runs to clear)
 * 2. Slower scaling: 3% per floor (allows meta-progression to keep pace)
 *
 * Final settings:
 * - 2 starting potions (100 HP emergency healing)
 * - Moderate XP curve (70 + level*50)
 * - Floor 1 enemies: +30% HP/ATK from original (Goblin 52 HP, 10 ATK)
 * - Better Hero Souls (5 + floor/2)
 * - 3% PER FLOOR SCALING ← KEY FIX
 * - 50 total floors
 */

console.log('🎮 ULTIMATE BALANCED SIMULATOR');
console.log('Harder Floor 1 + 3% floor scaling\n');
...
// ===== ENEMY BASE STATS (Floor 1) - Harder starting difficulty =====
const BASE_ENEMIES = {
...
function scaleEnemy(baseEnemy, floor) {
    const scaleFactor = 1 + (floor - 1) * 0.03; // 3% per floor (reduced from 5%)
    return {
...
console.log('\n🎮 PLAYER JOURNEY: Complete game simulation...\n');
...
while (highestFloorReached <= 50 && runNumber < 1000) {
    runNumber++;
...
// ===== FINAL ANALYSIS =====
console.log('\n' + '='.repeat(70));
console.log('📊 ULTIMATE BALANCE ANALYSIS\n');
...
if (firstClear) {
    if (firstClear.run <= 5) {
        ...
        console.log(`✅ Floor 1 PERFECT! (${firstClear.run} runs, ${firstClear.time}m)`);
    } else {
...
if (floor10) {
    ...
    if (h < 1) {
        ...
        console.log(`✅ Floor 10 PERFECT! (${h}h ${m}m - Target: 1-4h)`);
    } else {
...
if (fullComplete) {
    const h = Math.floor(fullComplete.time / 60), m = fullComplete.time % 60;
    if (h < 4) {
        console.log(`⚠️  Game too short (${h}h ${m}m)`);
    } else if (h <= 12) {
        console.log(`✅ PERFECT LENGTH! (${h}h ${m}m - Target: 6-12h)`);
    } else {
        console.log(`⚠️  Game too long (${h}h ${m}m)`);
    }
} else {
    console.log(`❌ Incomplete after ${runNumber} runs (${Math.floor(totalPlayTime / 3600)}h ${Math.floor((totalPlayTime % 3600) / 60)}m) - Stuck at Floor ${highestFloorReached - 1}`);
}
