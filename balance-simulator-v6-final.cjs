#!/usr/bin/env node
/**
 * Game Balance Simulator V6 FINAL - Orc Brute Nerf
 * Tests V.39.2 balance changes - Reduced Orc Brute ATK 15→13
 */

console.log('🎮 Balance Simulator V6 FINAL - Testing V.39.2 (Orc Brute Nerf)\n');
...
// Enemy stats (V.39.2 - Orc Brute ATK reduced from 15 to 13)
const enemies = {
    goblin: { hp: 48, atk: 10, def: 3, spd: 6, crit: 5 },
    slime: { hp: 54, atk: 9, def: 8, spd: 3, crit: 3 },
    orcBrute: { hp: 72, atk: 13, def: 5, spd: 5, crit: 8 }  // Reduced from 15 to 13
};
...
console.log('\n📊 Running 25 Simulations (V.39.2 FINAL - Orc Brute Nerf)...\n');
...
if (actualSurvival >= 55 && actualSurvival <= 75) {
    ...
    console.log('   Consider removing 1 starting potion or increasing enemy stats by 5%.');
} else if (actualSurvival < 45) {
    console.log('\n⚠️  WARNING: Too difficult');
    console.log('   Consider adding 1 more potion or reducing Orc Brute ATK by 1.');
} else {
...
console.log('Sample Matchups (V.39.2 FINAL - Orc Brute ATK 13):\n');
...
console.log('   - Enemy HP increased 20%, ATK increased ~25% (Orc Brute: 13)');
