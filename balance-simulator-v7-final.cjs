#!/usr/bin/env node
/**
 * Game Balance Simulator V7 FINAL - 3 Potions with Reduced HP
 * Tests V.39.2 balance - Final iteration with 3 potions + reduced player HP
 */

console.log('🎮 Balance Simulator V7 FINAL - V.39.2 (3 Potions + Reduced HP)\n');
console.log('='.repeat(70));

const characters = {
    warrior: { hp: 80, atk: 12, def: 10, spd: 6, crit: 10, resource: 50, potions: 3 },
    sorceress: { hp: 65, atk: 14, def: 6, spd: 7, crit: 12, resource: 70, potions: 3 },
    rogue: { hp: 72, atk: 15, def: 7, spd: 10, crit: 18, resource: 60, potions: 3 }
};
...
function simulateCombat(player, enemy) {
    ...
    return { won: playerHP > 0, playerHP: Math.max(0, playerHP), turns, damageTaken: playerDamageTaken };
}

function runFullSimulation(characterType, simNumber) {
    const player = { ...characters[characterType] };
    let totalCombats = 0, combatsWon = 0, totalDamageTaken = 0, deaths = 0, floorsCompleted = 0, potionsUsed = 0;
    ...
    for (let i = 0; i < floor1Combats; i++) {
        ...
        if (result.won) {
            ...
            if (hpPercent < 0.4 && player.potions > 0) {
...
console.log('\n📊 Running 25 Simulations (V.39.2 FINAL - 3 Potions)...\n');
...
const avgFinalHP = survivedSims.length > 0 ? Math.round(survivedSims.reduce((sum, s) => sum + s.finalHP, 0) / survivedSims.length) : 0;
...
if (survivedSims.length > 0) console.log(`Avg Final HP:       ${avgFinalHP} HP (survivors)`);
...
characterTypes.forEach(char => {
    ...
    const charAvgFinalHP = charSurvivors.length > 0 ? Math.round(charSurvivors.reduce((sum, s) => sum + s.finalHP, 0) / charSurvivors.length) : 0;
...
if (actualSurvival >= 55 && actualSurvival <= 75) {
    ...
    console.log('\n⚠️  WARNING: Slightly too easy - Consider -1 potion or +5% enemy stats.');
} else if (actualSurvival < 45) {
    console.log('\n⚠️  WARNING: Too difficult - Consider +1 potion or -5% enemy stats.');
} else {
    console.log('\n✅ GOOD: Balance is acceptable (45-55% survival)');
}
